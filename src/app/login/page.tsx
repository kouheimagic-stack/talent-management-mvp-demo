      ...
      <div className="mx-auto grid min-h-[calc(100vh-48px)] max-w-6xl items-center gap-6 sm:min-h-[calc(100vh-80px)] lg:grid-cols-[1fr_460px] lg:gap-10">
        <section>
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white/80 p-4 text-sm text-slate-600 shadow-sm backdrop-blur sm:mt-8">
            ...
            <p className="mt-2">MVP 0確認用のローカルデモユーザーを使用してください。</p>
            <p className="mt-1 text-xs text-slate-500">
              例: employee@example.com / password123
            </p>
          </div>
        </section>
        ...
        <Card className="shadow-xl shadow-slate-200/60">
          ...
          <CardContent className="space-y-5">
            <div className="rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm leading-6 text-sky-800">
              現在はMVP 0の安定確認用として、外部DBに接続せずローカルデモユーザーでログインします。
            </div>
            {params.error ? (
...
function getLoginErrorMessage(error: string) {
  const messages: Record<string, string> = {
    ...
    missing_profile: "ログインユーザー情報が見つかりません。",
    missing_employee: "社員情報が見つかりません。",
    db_connection: "データ取得に失敗しました。",
    session_expired: "ログイン状態を確認できませんでした。もう一度ログインしてください。",
    supabase_config: "外部サービス設定が未完了です。",
  };
